function* algorithm() {
    ...
    // the word we are going to visualize
    var WORD = "ALGORITHMS";

    // get bounds of surface we are displayed on
    var bounds = algo.SURFACE.root.getBounds();
    ...
    var varray = new algo.core.Array({
        ...
        createElement: _.bind(function (value, index) {
            ...
            // display the index in the tile as well
            element.valueElement.set({
                text: index
            });
        ...
        swapElement: _.bind(function (value, newIndex, oldIndex, element) {
            ...
            var newCell = layout.getBox(0, newIndex), oldCell = layout.getBox(0, oldIndex);

            // get x position for element centered in cell
            var newX = newCell.cx - element.w / 2, oldX = oldCell.cx - element.w / 2;
            ...
            var H = element.h * 1.5, Y = element.y;
    ...
    while (left < right) {
        ...
        yield({
            step       : "Exchange the items current indexes by the left and right indices.",
            variables  : {
                left : left,
                right: right
            }
    ...
    // show all items as in their correct places.

    algo.SURFACE.root.set({state: algo.render.kS_FADED});
